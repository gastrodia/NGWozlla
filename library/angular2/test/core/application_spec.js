...
var HelloRootCmpContent = (function () {
    ...
    HelloRootCmpContent = __decorate([
        annotations_1.Component({ selector: 'hello-app' }),
        annotations_1.View({ template: 'before: <ng-content></ng-content> after: done' }), 
        __metadata('design:paramtypes', [])
    ], HelloRootCmpContent);
...
var HelloRootCmp4 = (function () {
    ...
    HelloRootCmp4 = __decorate([
        annotations_1.Component({ selector: 'hello-app' }),
        annotations_1.View({ template: '' }),
        __param(0, di_1.Inject(core_1.LifeCycle)), 
        __metadata('design:paramtypes', [Object])
    ], HelloRootCmp4);
...
function main() {
    ...
    test_lib_1.describe('bootstrap factory method', function () {
        test_lib_1.beforeEach(function () {
            ...
            testBindings = [di_1.bind(render_1.DOCUMENT_TOKEN).toValue(fakeDoc)];
        });
        test_lib_1.it('should throw if bootstrapped Directive is not a Component', test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            ...
            var exceptionHandler = new exception_handler_1.ExceptionHandler(logger, false);
            var refPromise = application_1.bootstrap(HelloRootDirectiveIsNotCmp, [testBindings, di_1.bind(exception_handler_1.ExceptionHandler).toValue(exceptionHandler)]);
            async_1.PromiseWrapper.then(refPromise, null, function (exception) {
                test_lib_1.expect(exception).toContainError("Could not load '" + lang_1.stringify(HelloRootDirectiveIsNotCmp) + "' because it is not a component.");
                test_lib_1.expect(logger.res.join("")).toContain("Could not load");
                async.done();
                return null;
            });
        }));
        test_lib_1.it('should throw if no element is found', test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            var logger = new _ArrayLogger();
            var exceptionHandler = new exception_handler_1.ExceptionHandler(logger, test_lib_1.IS_DARTIUM ? false : true);
            var refPromise = application_1.bootstrap(HelloRootCmp, [di_1.bind(exception_handler_1.ExceptionHandler).toValue(exceptionHandler)]);
        ...
        test_lib_1.it("should avoid cyclic dependencies when root component requires Lifecycle through DI", test_lib_1.inject([test_lib_1.AsyncTestCompleter], function (async) {
            ...
            refPromise.then(function (ref) {
                test_lib_1.expect(ref.hostComponent.lc).toBe(ref.injector.get(core_1.LifeCycle));
                async.done();
            });
        }));
