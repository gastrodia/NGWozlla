...
function main() {
    ...
    test_lib_1.describe('EventManager', function () {
        test_lib_1.it('should delegate event bindings to plugins', function () {
            ...
            test_lib_1.expect(plugin._nonBubbleEventHandlers.get('click')).toBe(handler);
        });
        test_lib_1.it('should delegate bubbling events to plugins', function () {
            ...
            var manager = new event_manager_1.EventManager([plugin, domEventPlugin], new FakeNgZone());
            manager.addEventListener(element, '^click', handler);
            test_lib_1.expect(plugin._bubbleEventHandlers.get('click')).toBe(handler);
        });
        test_lib_1.it('should delegate event bindings to the first plugin supporting the event', function () {
            ...
            test_lib_1.expect(plugin1._nonBubbleEventHandlers.has('click')).toBe(false);
            test_lib_1.expect(plugin2._nonBubbleEventHandlers.get('click')).toBe(clickHandler);
            test_lib_1.expect(plugin2._nonBubbleEventHandlers.has('dblclick')).toBe(false);
            test_lib_1.expect(plugin1._nonBubbleEventHandlers.get('dblclick')).toBe(dblClickHandler);
        });
...
var FakeEventManagerPlugin = (function (_super) {
    ...
    function FakeEventManagerPlugin(supports) {
        ...
        this._nonBubbleEventHandlers = new collection_1.Map();
        this._bubbleEventHandlers = new collection_1.Map();
    }
