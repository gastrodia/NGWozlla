...
function main() {
    test_lib_1.describe('DirectiveParser', function () {
        ...
        function process(el, propertyBindings, directives) {
            ...
            return collection_1.ListWrapper.map(pipeline.processElements(el, api_1.ViewType.COMPONENT, createViewDefinition()), function (ce) { return ce.inheritedElementBinder; });
        }
        ...
        test_lib_1.it('should compile children by default', function () {
            var results = createPipeline().processElements(test_lib_1.el('<div some-decor></div>'), api_1.ViewType.COMPONENT, createViewDefinition());
        ...
        test_lib_1.it('should stop compiling children when specified in the directive config', function () {
            var results = createPipeline().processElements(test_lib_1.el('<div some-decor-ignoring-children></div>'), api_1.ViewType.COMPONENT, createViewDefinition());
        ...
        test_lib_1.it('should bind directive properties from bound properties', function () {
            ...
            test_lib_1.expect(directiveBinding.propertyBindings.get('dirProp').source).toEqual('someExpr');
        });
        ...
        test_lib_1.it('should throw when parsing invalid host properties', function () {
            test_lib_1.expect(function () { return process(test_lib_1.el('<input some-decor-with-invalid-host-props>')); })
                .toThrowError(new RegExp('Simple binding expression can only contain field access and constants'));
        });
        ...
        test_lib_1.it('should read attribute values', function () {
            ...
            var results = process(element);
            test_lib_1.expect(results[0].readAttributes.get('some-attr')).toEqual('someValue');
        });
...
var someComponentDup = api_1.DirectiveMetadata.create({ selector: 'some-comp', id: 'someComponentDup', type: api_1.DirectiveMetadata.COMPONENT_TYPE });
var someComponent2 = api_1.DirectiveMetadata.create({ selector: 'some-comp2', id: 'someComponent2', type: api_1.DirectiveMetadata.COMPONENT_TYPE });
var someDirective = api_1.DirectiveMetadata.create({ selector: '[some-decor]', id: 'someDirective', type: api_1.DirectiveMetadata.DIRECTIVE_TYPE });
var someDirectiveIgnoringChildren = api_1.DirectiveMetadata.create({
    selector: '[some-decor-ignoring-children]',
    compileChildren: false,
    type: api_1.DirectiveMetadata.DIRECTIVE_TYPE
});
var decoratorWithMultipleAttrs = api_1.DirectiveMetadata.create({
    selector: 'input[type=text][control]',
    id: 'decoratorWithMultipleAttrs',
    type: api_1.DirectiveMetadata.DIRECTIVE_TYPE
});
var someDirectiveWithProps = api_1.DirectiveMetadata.create({
    selector: '[some-decor-props]',
    properties: ['dirProp: elProp'],
    readAttributes: ['some-attr']
});
var someDirectiveWithHostProperties = api_1.DirectiveMetadata.create({
    selector: '[some-decor-with-host-props]',
    host: collection_1.MapWrapper.createFromStringMap({ '[hostProp]': 'dirProp' })
});
var someDirectiveWithInvalidHostProperties = api_1.DirectiveMetadata.create({
    selector: '[some-decor-with-invalid-host-props]',
    host: collection_1.MapWrapper.createFromStringMap({ '[hostProp]': 'dirProp + dirProp2' })
});
var someDirectiveWithHostAttributes = api_1.DirectiveMetadata.create({
    selector: '[some-decor-with-host-attrs]',
    host: collection_1.MapWrapper.createFromStringMap({ 'attr_name': 'attr_val', 'class': 'foo bar' })
});
var someDirectiveWithEvents = api_1.DirectiveMetadata.create({
    selector: '[some-decor-events]',
    host: collection_1.MapWrapper.createFromStringMap({ '(click)': 'doIt()' })
});
var someDirectiveWithGlobalEvents = api_1.DirectiveMetadata.create({
    selector: '[some-decor-globalevents]',
    host: collection_1.MapWrapper.createFromStringMap({ '(window:resize)': 'doItGlobal()' })
});
var componentWithNonElementSelector = api_1.DirectiveMetadata.create({
